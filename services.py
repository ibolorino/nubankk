class Operation:
    ...
    def _update_quantity(self, order: dict) -> None:
        if order.get("operation") == "buy":
            self.quantity += order.get("quantity")
        else:
            self.quantity -= order.get("quantity")

    def _update_loss(self, order: dict, order_result: float) -> None:
        if order.get("operation") == "sell":
            self.loss += order_result
            if self.loss > 0:
                self.loss = 0

    def _update_taxes(self, order: dict, order_result: float) -> None:
